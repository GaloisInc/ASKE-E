...
struct SIR {
  ...
  double event_rate_Superspread() {
    return ((4.0e-4 * S) * I) / ((S + R) + I);
  }
  ...
  void next_event(int &nextEvent, double &nextTime) {
    ...
    double _eff_rate_Superspread = event_when_Superspread() ? event_rate_Superspread() : 0.0;
    double total_rate = _eff_rate_Infect + _eff_rate_Remove + _eff_rate_Superspread;
